"""
Re-run p1_classic at the default from-scratch finetune LR 0.025 (instead of the
...
def main():
    ...
    for tc in tcs:
        ...
        res = run_protograph_variant(
            "p1_classic", proto_walks["p1"], inst_walks, p["ontology"],
            p["train"], p["test"],
        ...
        print(f"{tc}: p1_classic@lr0.025 final={res['final_acc']:.3f}  [{accs}]", flush=True)
        results[tc] = res["final_acc"]

    print("\nFINAL:")
    print(json.dumps({tc: round(v, 3) for tc, v in results.items()}, indent=2))
    (OUT_ROOT / "p1_classic_lr025.json").write_text(
