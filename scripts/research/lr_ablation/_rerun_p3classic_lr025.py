"""
Re-run p3_classic at the from-scratch finetune LR 0.025 (instead of the protected
0.0025) for tc01-tc15, using the EXACT synthetic_compare pipeline and its cached
walks, so the numbers are directly comparable to p1_classic_lr025.json /
p2_classic_lr025.json and to the other columns of Table tab:lr_ablation.

Mirrors scripts/_rerun_p1classic_lr025.py; only the protograph kind (p3) and the
strategy (all_init, the classic own-class-mean rule used for p3_classic) differ.
"""
...
CFG = dict(
    ...
    epochs=5,
    pretrain_epochs=5,
    finetune_alpha=0.025,   # <-- the change: from-scratch LR, not the protected 0.0025
    min_alpha=0.0001,
    target_norm=8.0,
    seed=42,
)
...
def main():
    ...
    for tc in tcs:
        ...
        proto_walks = ensure_walks(
            proto_paths["p3"], out / "walks_p3.txt",
        ...
        res = run_protograph_variant(
            "p3_classic", proto_walks, inst_walks, p["ontology"],
            p["train"], p["test"],
        ...
        print(f"{tc}: p3_classic@lr0.025 final={res['final_acc']:.3f}  [{accs}]", flush=True)
        results[tc] = res["final_acc"]

    print("\nFINAL:")
    print(json.dumps({tc: round(v, 3) for tc, v in results.items()}, indent=2))
    (OUT_ROOT / "p3_classic_lr025.json").write_text(
